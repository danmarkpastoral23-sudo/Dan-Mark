...
- EXACT same styles, table format, header/footer as original
- Adds missing sections: Glossary, Equipment Specs, Foundation Calcs,
  Facility Inventory, Energy Balance, O&M Plan, Materials/Color Coding
- Placeholder sections for Plant Layout and 3D Model images
"""
import zipfile, os
...
with zipfile.ZipFile(ORIGINAL) as z:
    ...
    img1 = z.read("word/media/image1.jpg")  # PSU logo small
    img2 = z.read("word/media/image2.jpg")  # PSU logo wide
    img3 = z.read("word/media/image3.png")  # Fig II-B.1
    img4 = z.read("word/media/image4.png")  # Fig II-B.2
    img5 = z.read("word/media/image5.png")  # Fig II-B.3

# ── Namespaces (exact same as original) ──────────────────────────────
NS = (
...
# ── XML helpers — exact same look as original ─────────────────────────
...
def p(text="", style="Normal", bold=False, italic=False,
    ...
    if runs is not None:
        ...
        body = f'<w:r>{rpr}<w:t xml:space="preserve">{xe(text)}</w:t></w:r>' if text else ""
...
def body(text, sz="20"): return p(text, sz=sz)
def blank(): return p("")
...
def tbl(rows, widths=None, header=True):
    """Exact same table style as original:
    - Header: #1B6B3A bg, white bold centered text
    - Data rows: #F1F8E9 bg, #CCCCCC borders
    """
    if not rows: return ""
    nc = max(len(r) for r in rows)
    total_w = 9360
    if widths is None:
        widths = [total_w // nc] * nc
    ...
    tbl_pr = (
        ...
        f'</w:tblBorders>'
        f'<w:tblCellMar>'
        f'<w:left w:w="10" w:type="dxa"/><w:right w:w="10" w:type="dxa"/>'
        f'</w:tblCellMar>'
    ...
    for ri, row in enumerate(rows):
        ...
        for ci in range(nc):
            ...
            tcpr = (
                f'<w:tcPr><w:tcW w:w="{widths[ci]}" w:type="dxa"/>'
                f'<w:tcBorders>'
                f'<w:top w:val="single" w:sz="{"4" if is_hdr else "1"}" w:space="0" w:color="{border_color}"/>'
                f'<w:left w:val="single" w:sz="{"4" if is_hdr else "1"}" w:space="0" w:color="{border_color}"/>'
                f'<w:bottom w:val="single" w:sz="{"4" if is_hdr else "1"}" w:space="0" w:color="{border_color}"/>'
                f'<w:right w:val="single" w:sz="{"4" if is_hdr else "1"}" w:space="0" w:color="{border_color}"/>'
                f'</w:tcBorders>'
            ...
            if is_hdr:
                cell_content = (
                    ...
                    f'<w:t>{xe(cell)}</w:t></w:r></w:p>'
                )
            else:
                cell_content = (
                    f'<w:p><w:r><w:rPr>'
                    f'<w:sz w:val="20"/><w:szCs w:val="20"/></w:rPr>'
                    f'<w:t xml:space="preserve">{xe(cell)}</w:t></w:r></w:p>'
                )

            out.append(f"<w:tc>{tcpr}{cell_content}</w:tc>")
        out.append("</w:tr>")
...
def note(text):
    """Italic note paragraph, same size as body."""
    return (
        '<w:p><w:r><w:rPr><w:i/><w:sz w:val="18"/><w:szCs w:val="18"/>'
        f'<w:color w:val="595959"/></w:rPr>'
        f'<w:t xml:space="preserve">{xe(text)}</w:t></w:r></w:p>'
    )

def img_inline(rId, cx, cy, img_id, name):
...
def img_placeholder(label, width_cm=15, height_cm=10):
    """Gray box placeholder for images to be inserted later."""
    cx = int(width_cm * 914400 / 2.54)
    cy = int(height_cm * 914400 / 2.54)
    # Use a text placeholder paragraph instead of actual image
    return (
        ...
        f'<w:t>[ {xe(label)} ]</w:t></w:r></w:p>'
        f'<w:p><w:pPr><w:jc w:val="center"/></w:pPr>'
        f'<w:r><w:rPr><w:i/><w:color w:val="888888"/><w:sz w:val="18"/><w:szCs w:val="18"/></w:rPr>'
        f'<w:t>Insert image here — approximately {width_cm} cm × {height_cm} cm</w:t></w:r></w:p>'
    )
...
# ── Title page logos (same as original)
A(
    '<w:p><w:pPr><w:jc w:val="center"/></w:pPr>'
    '<w:r><w:drawing><wp:inline distT="0" distB="0" distL="0" distR="0">'
    '<wp:extent cx="1097280" cy="1107255"/><wp:effectExtent l="0" t="0" r="0" b="0"/>'
    ...
    '<pic:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="1097280" cy="1107255"/></a:xfrm>'
    '<a:prstGeom prst="rect"><a:avLst/></a:prstGeom></pic:spPr>'
    '</pic:pic></a:graphicData></a:graphic>'
    '</wp:inline></w:drawing></w:r>'
    '<w:r><w:t xml:space="preserve">  </w:t></w:r>'
    '<w:r><w:drawing><wp:inline distT="0" distB="0" distL="0" distR="0">'
    '<wp:extent cx="1371600" cy="857057"/><wp:effectExtent l="0" t="0" r="0" b="0"/>'
    ...
    '<pic:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="1371600" cy="857057"/></a:xfrm>'
    '<a:prstGeom prst="rect"><a:avLst/></a:prstGeom></pic:spPr>'
    '</pic:pic></a:graphicData></a:graphic>'
    '</wp:inline></w:drawing></w:r></w:p>'
)

A(p("Pangasinan State University", bold=True, align="center", sz="24"))
A(p("Urdaneta City Campus", align="center", sz="22"))
A(p("College of Engineering and Architecture", align="center", sz="22"))
A(p("Mechanical Engineering Department", align="center", sz="22"))
A(p("Second Semester, A.Y. 2025–2026", align="center", sz="22"))
A(blank())
A(p("WINDMILL PROJECT PROPOSAL", style="Heading1",
    runs=[run("WINDMILL PROJECT PROPOSAL", bold=True, sz="44", color="2E74B5")]))
A(p("Pasuquin, Ilocos Norte, Philippines", align="center", sz="26"))
A(p("SUPPLEMENTAL ENGINEERING DOCUMENTATION", align="center", sz="22", italic=True))
A(blank())
A(p("Submitted by:", bold=True, align="center", sz="22"))
A(p("Dan Mark C. Pastoral", align="center", sz="22", bold=True))
A(p("BSME-4A", align="center", sz="22"))
A(blank())
A(p("Submitted to:", bold=True, align="center", sz="22"))
A(p("Marfel D. Rosario, PME", align="center", sz="22", bold=True))
A(p("Instructor", align="center", sz="22"))
A(blank())
A(p("May 2026", align="center", sz="22"))
A(pb())



# ════════════════════════════════════════════════════════════════════════
# SECTION VIII-A — GLOSSARY OF TERMS AND ACRONYMS
# (missing from original — present in comparison doc)
# ════════════════════════════════════════════════════════════════════════
A(h1("Section VIII-A — Glossary of Terms and Acronyms"))
A(body("The following technical terms and acronyms appear throughout this proposal. "
...
A(tbl([
    ...
    ["EPC", "Engineering, Procurement and Construction — the main project delivery contract mode."],
    ["ERC", "Energy Regulatory Commission — approves FiT rates, grid codes, and metering standards."],
    ["FiT", "Feed-in Tariff — guaranteed PHP 8.53/kWh paid to qualified wind energy generators (ERC Res. 16, 2022)."],
    ["FPIC", "Free, Prior and Informed Consent — required under RA 8371 (IPRA) for indigenous communities."],
    ...
    ["Rated Speed", "Wind speed at which the turbine first reaches its rated output. This project: 12–14 m/s."],
    ...
    ["Weibull Dist.", "Statistical model for wind speed frequency; defined by shape parameter k and scale c."],
    ["Wind Class II", "IEC classification for sites with average wind speed 7.5–8.5 m/s — Pasuquin's class."],
    ["Wind Power Density", "Available wind power per unit area: WPD = ½ρV³ (W/m²)."],
    ["Wind Shear", "Increase of wind speed with altitude: V(z) = V_ref × (z/z_ref)^α; α ≈ 0.14 coastal."],
], widths=[2600, 6760]))
A(pb())


# ════════════════════════════════════════════════════════════════════════
# SECTION VIII-B — DETAILED EQUIPMENT SPECIFICATIONS
# ════════════════════════════════════════════════════════════════════════
A(h1("Section VIII-B — Detailed Equipment Specifications"))
A(body("Equipment specifications below are based on the 5.0 MW HAWT, 130 m rotor, "
       "100 m hub height, and 33/115 kV electrical system established in Section III "
       "of the original proposal (20-turbine × 5 MW = 100 MW configuration)."))
A(blank())
...
A(tbl([
    ...
    ["Gearbox lubrication", "Forced ISO VG 320 mineral/PAO synthetic oil, 600 L sump"],
...
A(tbl([
    ...
    ["Coating", "ISO 12944 C5-M: zinc-rich primer + epoxy midcoat + PU topcoat, DFT ≥ 320 µm"],
...
A(tbl([
    ...
    ["Substation protection", "IEC 61850 relays: 87T differential, 21 distance, 51 overcurrent, 64 REF"],
...
# ════════════════════════════════════════════════════════════════════════
# SECTION VIII-C — PLANT LAYOUT AND 3D MODEL
# ════════════════════════════════════════════════════════════════════════
A(h1("Section VIII-C — Plant Layout and 3D Model"))
A(body("This section presents the wind farm site layout and a three-dimensional "
       "visualization of the Pasuquin Wind Farm. The layout is based on the turbine "
       "spacing calculations in Section III-B.2 and the siting factors in Section IV."))
A(blank())

A(h2("C.1 Plant Site Layout"))
A(body("The plant layout shows the arrangement of 20 wind turbine generators (WTG-101 "
       "to WTG-120) across the Pasuquin coastline and ridges, with the on-site substation, "
       "access roads, and ancillary facilities. Turbines are arranged in 5 rows of 4 units "
       "each, with 7D (910 m) inline spacing and 4D (520 m) cross-wind spacing per "
       "Section III-B.1 wake effect calculations."))
A(blank())
A(img_placeholder("PLANT LAYOUT — Site plan showing turbine positions, substation, access roads, and facilities", 16, 12))
A(caption("Figure VIII-C.1 — Pasuquin Wind Farm Site Layout Plan"))
A(blank())

A(h2("C.2 3D Model / Rendering"))
A(body("The three-dimensional model below illustrates the proposed wind farm as it "
       "would appear along the Pasuquin coastline, showing the relative scale of the "
       "turbines (100 m hub height, 130 m rotor diameter) against the terrain, "
       "the substation at the array centroid, and the eco-tourism visitor area."))
A(blank())
...
A(note("Note: Insert finalized layout drawing and 3D rendering above. "
       "Recommended format: PNG or JPG, minimum 300 DPI, landscape orientation. "
       "The placeholder boxes indicate the approximate size and position of each figure."))
A(pb())


# ════════════════════════════════════════════════════════════════════════
# SECTION VIII-D — FOUNDATION AND ANCHOR BOLT CALCULATIONS
# ════════════════════════════════════════════════════════════════════════
A(h1("Section VIII-D — Foundation and Anchor Bolt Calculations"))
A(body("This section sizes the gravity foundation and anchor bolt cage for each "
       "turbine tower based on loads from IEC 61400-1 Class II and checked per NSCP 2015."))
A(blank())
...
A(body("A cast-in-place reinforced concrete octagonal slab resists overturning by self-weight."))
A(tbl([
    ...
    ["Reinforcement grade", "Grade 60 (fy = 414 MPa)", "Top + bottom mats, both directions"],
    ...
    ["Required allowable soil bearing capacity", "≥ 250 kPa", "To be confirmed by geotechnical investigation"],
], widths=[4000, 2600, 2760]))
A(blank())

A(h2("D.3 Anchor Bolt Cage"))
A(body("Anchor bolts transfer bending moment between the steel tower base flange and "
       "the concrete foundation. Double-ring arrangement (inner + outer)."))
A(tbl([
...
A(note("NOTE: All values above are preliminary. Final foundation and anchor bolt design must be "
       "certified by a Philippine-licensed Civil/Structural Engineer based on geotechnical "
       "investigation and actual turbine load report from the equipment supplier."))
A(pb())


# ════════════════════════════════════════════════════════════════════════
# SECTION VIII-E — FACILITY INVENTORY AND FLOOR AREAS
# ════════════════════════════════════════════════════════════════════════
A(h1("Section VIII-E — Facility Inventory and Floor Areas"))
A(body("Complete list of permanent and temporary facilities based on the plant layout "
       "in Section IV. Land between turbines (~9.45 km²) remains usable for farming and grazing."))
A(blank())
A(tbl([
    ...
    ["4", "Maintenance workshop and stores", "650", "Spare parts, gearbox oil, blade tools, crane access"],
    ...
    ["7", "Emergency and fire station", "240", "Nacelle fire response, high-angle rescue equipment"],
    ["8", "Diesel genset house + fuel storage", "120", "500 kVA backup + 5,000 L diesel fuel tank"],
    ...
    ["11", "Parking and crane turnaround area", "1,200", "Heavy vehicles, service vans, mobile crane setup"],
    ["12", "Material laydown / crane hardstand", "2,500", "Blade, nacelle, tower section staging (construction)"],
    ["13", "Septic and sewage treatment", "150", "On-site sanitary waste treatment"],
    ["14", "Water tank and pump house", "80", "Blade wash water, fire-reserve tank (50,000 L)"],
    ["15", "Internal access roads (7 km × 6 m wide)", "≈ 42,000", "Heavy-haul gravel roads to each turbine"],
    ...
    ["", "GROSS WIND FARM AREA (including inter-turbine land)", "≈ 9.46 km²", "Per wake spacing calc in Sec. III-B.2"],
], widths=[600, 3800, 2400, 2560]))
A(pb())


# ════════════════════════════════════════════════════════════════════════
# SECTION VIII-F — ENERGY BALANCE DIAGRAM
# ════════════════════════════════════════════════════════════════════════
A(h1("Section VIII-F — Energy Balance Diagram"))
A(body("This section extends the Process Flow Diagrams in Section II-B by quantifying "
       "the power conversion losses at each stage — from incident wind kinetic energy "
       "to electricity delivered at the NGCP grid metering point."))
A(blank())

A(h2("F.1 Stage-by-Stage Power Balance (per turbine at rated wind V = 12 m/s)"))
A(body("Calculation basis: P_wind = ½ × ρ × A × V³ = ½ × 1.225 × 13,273 × 12³ = 13.96 MW "
       "(same Betz equation used in Section III-A.1 of the original proposal)."))
A(blank())
A(tbl([
    ["Stage", "Input (MW)", "Loss (MW)", "Output (MW)", "Loss Mechanism"],
    ["Wind kinetic energy at rotor disc", "13.96", "—", "13.96", "P_wind = ½ρAV³"],
    ["Aerodynamic conversion (Cp = 0.45)", "13.96", "7.68", "6.28", "Real rotor < Betz 59.3% theoretical max"],
    ["Hub + main bearing friction", "6.28", "0.06", "6.22", "≈ 1% mechanical friction losses"],
    ["Gearbox (3-stage planetary, η = 97%)", "6.22", "0.19", "6.03", "≈ 3% gear mesh and bearing losses"],
    ["DFIG generator (η = 97%)", "6.03", "0.18", "5.85", "≈ 3% electromagnetic losses"],
    ["Partial-scale power converter", "5.85", "0.10", "5.75", "≈ 1.7% IGBT switching losses"],
    ["Pad-mount transformer 690 V / 33 kV", "5.75", "0.05", "5.70", "≈ 0.9% no-load + load losses"],
    ["Inter-array 33 kV cable (average)", "5.70", "0.04", "5.66", "≈ 0.7% I²R resistive losses"],
    ["Step-up transformer 33 kV / 115 kV", "5.66", "0.03", "5.63", "≈ 0.5% transformer losses"],
    ["115 kV OHL to Laoag (≈ 35 km)", "5.63", "0.05", "5.58", "≈ 0.9% line resistive losses"],
    ["NET at NGCP grid metering point", "—", "—", "5.58", "Useful electricity delivered to grid"],
...
A(tbl([
    ...
    ["Maximum theoretical AEP per turbine", "43,800 MWh/yr", "5.0 MW × 8,760 h/yr"],
    ["Capacity factor — Pasuquin site", "38%", "Consistent with Bangui 35–42% and Burgos 36–45%"],
    ["Gross AEP per turbine", "16,644 MWh/yr", "43,800 × 0.38"],
    ["Electrical system losses (avg. 7%)", "1,165 MWh/yr", "Cables, transformers, transmission line"],
    ["Net AEP per turbine delivered to grid", "15,479 MWh/yr", "16,644 × (1 − 0.07)"],
    ["Wind farm total (20 turbines)", "309,580 MWh/yr", "15,479 × 20 turbines"],
    ["Equivalent households served", "≈ 77,400 households", "At 4,000 kWh/yr per household (PH average)"],
    ["Annual FiT revenue (gross)", "PHP 2.64 Billion/yr", "309,580 MWh × PHP 8.53/kWh (ERC FiT-2)"],
], widths=[3200, 2400, 3760]))
A(pb())


# ════════════════════════════════════════════════════════════════════════
# SECTION VIII-G — OPERATIONS AND MAINTENANCE PLAN
# ════════════════════════════════════════════════════════════════════════
A(h1("Section VIII-G — Operations and Maintenance (O&M) Plan"))
A(body("A rigorous O&M plan sustains the 38% capacity factor assumed in the financial "
       "analysis (Section VI) across the 25-year project life."))
A(blank())
...
A(tbl([
    ...
    ["Weekly", "Ground-level visual inspection; drone oil-leak check on nacelle"],
    ["Monthly", "Tower climb: bolt torque spot-check, lubrication top-up, brake pad inspection"],
    ["Quarterly", "Gearbox oil spectroscopy sample; blade leading-edge visual check"],
    ["Semi-annual", "Yaw and pitch function test; emergency descender re-certification"],
    ["Annual", "Full PM: oil change, filter change, blade root bolt re-torque (144 anchor bolts)"],
...
for s in [
    "Tier A — On-site (immediate use): pitch motors, yaw motors, fuses, contactors, sensors, lubricants, filters.",
    "Tier B — Regional depot (24-hour delivery): IGBT converter modules, slip rings, brake pads, hydraulic accumulators.",
    "Tier C — Manufacturer warehouse (2–6 weeks): main bearing, gearbox sub-assembly, generator, blade section.",
]:
    A(bullet(s))
A(blank())

A(h2("G.4 Safety and Compliance Requirements"))
for s in [
    "All turbine technicians must hold a valid GWO Basic Safety Training (BST) certificate: Working at Heights, First Aid, Manual Handling, Fire Awareness.",
    "Two-person rule enforced for any work at height above 30 m inside or outside the tower.",
    "Hot-work permits required for welding or grinding inside the nacelle.",
    "Annual emergency drill: nacelle fire suppression, tower rescue, medical evacuation.",
    "All work performed in compliance with DOLE Department Order No. 198-18 (Occupational Safety and Health Standards).",
]:
    A(bullet(s))
A(pb())
...
# ════════════════════════════════════════════════════════════════════════
# SECTION VIII-H — MATERIALS AND COLOR-CODING STANDARDS
# ════════════════════════════════════════════════════════════════════════
...
A(tbl([
    ...
    ["Tower flange bolts", "ISO 898-1 Class 10.9, hot-dip galvanized (HDG)"],
    ...
    ["External coating", "ISO 12944 C5-M system: zinc-rich primer + epoxy mid + PU topcoat, total DFT ≥ 320 µm"],
    ["Blade composite", "Glass-fiber reinforced epoxy (GFRP) + carbon-fiber spar caps; IEC 61400-5"],
    ...
    ["Control cabinets", "IP66 painted steel enclosures; RAL 7035 light grey"],
    ["Gearbox lubricant", "ISO VG 320 mineral or PAO synthetic oil"],
    ["Yaw/pitch bearing grease", "NLGI-2 lithium-complex grease with EP additive"],
], widths=[2800, 6560]))
A(blank())

A(h2("H.2 Color-Coding Scheme"))
A(tbl([
    ["Item", "Color (RAL Code)", "Standard / Reason"],
    ["Tower body — lower 2/3", "RAL 9010 Pure White", "IEC / aviation standard baseline"],
    ["Tower body — upper 1/3 (2 red bands)", "RAL 3020 Traffic Red", "CAAP MOS Part 14 — daytime obstruction marking"],
    ["Blade tips (last 6 m, alternating bands)", "RAL 3020 + RAL 9010", "Aviation visibility"],
    ["Aviation warning lights", "Red, medium-intensity flashing", "ICAO Annex 14 / CAAP rules"],
    ["Earthing / grounding conductors", "Green/Yellow striped insulation", "IEC 60446"],
    ["33 kV cable phase identification", "L1 = Red, L2 = Yellow, L3 = Blue", "PEC 2017 Philippine standard"],
    ["Fire-fighting equipment", "RAL 3000 Flame Red", "DOLE OSH standards"],
    ["Emergency exits and evacuation routes", "RAL 6024 Traffic Green + white text", "ISO 7010"],
    ["115 kV substation control panels", "RAL 7032 Pebble Grey", "Philippine substation convention"],
    ["Hazard / caution floor markings", "RAL 1023 Traffic Yellow + black diagonal stripes", "ISO 3864"],
...
# CLOSING
# ════════════════════════════════════════════════════════════════════════
A(h1("Note to Instructor"))
A(body("This Supplemental Engineering Documentation (Sections VIII-A through VIII-H) "
       "complements the original Windmill Project Proposal (April 29, 2026). "
       "Together, the two documents provide a complete plant-design submission covering: "
       "site selection, wind resource assessment, technical specifications and "
       "engineering calculations, process flow diagrams, plant siting and layout, "
       "environmental impact assessment, financial analysis, glossary of terms, "
       "detailed equipment specifications, plant layout and 3D model (placeholders "
       "provided in Section VIII-C for final drawings), foundation calculations, "
       "facility inventory, energy balance, O&M plan, and materials/color-coding standards."))
A(blank())
A(p(runs=[run("Prepared by: ", bold=True, sz="20"),
          run("Dan Mark C. Pastoral  ·  BSME-4A  ·  Pangasinan State University", sz="20")]))
A(p(runs=[run("Submitted to: ", bold=True, sz="20"),
          run("Marfel D. Rosario, PME — Instructor", sz="20")]))
A(p(runs=[run("Date: ", bold=True, sz="20"),
          run("May 2026", sz="20")]))
...
with zipfile.ZipFile(OUTPUT, "w", zipfile.ZIP_DEFLATED) as z:
    ...
    z.writestr("word/media/image1.jpg", img1)
    z.writestr("word/media/image2.jpg", img2)
    z.writestr("word/media/image3.png", img3)
    z.writestr("word/media/image4.png", img4)
    z.writestr("word/media/image5.png", img5)

size_kb = os.path.getsize(OUTPUT) // 1024
print(f"✅  {OUTPUT}  ({size_kb} KB)")
print(f"   body elements : {len(B)}")
print(f"   document.xml  : {len(document_xml):,} bytes")
print(f"   images embedded: image1-5 (logos + 3 process flow figs)")
